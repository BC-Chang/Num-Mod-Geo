...
% u = Nx by 1 vector of temperature in cell centers.
...
%
% Output:
% q = Nfx by 1 vector of heat fluxes across all cell faces,
%
% Example call:
...
L = -D*Kd*G;
res   = L*u - fs;   % column vector of residual in all cells 
...
Xmin = ismember(dof_faces,Grid.dof_f_xmin);
Xmax = -ismember(dof_faces,Grid.dof_f_xmax);

sign = Xmin + Xmax; % column vector of flips the sign on xmax, ymax, zmax (in 3D) bnd's

q(dof_faces) =  sign.*res_bnd.*Grid.V(dof_cells)./Grid.A(dof_faces);
