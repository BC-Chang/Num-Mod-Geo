...
Grid.dof_ymin_vy = Grid.x.N + Grid.y.dof_ymin;
Grid.dof_ymax_vy = Grid.x.N + Grid.y.dof_ymax;

% Tangential velocities on bnd's
Grid.dof_xmin_vy = Grid.x.N + Grid.y.dof_xmin;
Grid.dof_xmax_vy = Grid.x.N + Grid.y.dof_xmax;
Grid.dof_ymin_vx = Grid.x.dof_ymin;
Grid.dof_ymax_vx = Grid.x.dof_ymax;
% excluding extreme faces
Grid.dof_xmin_vt = Grid.dof_xmin_vy(2:end-1); 
Grid.dof_xmax_vt = Grid.dof_xmax_vy(2:end-1);
Grid.dof_ymin_vt = Grid.dof_ymin_vx(2:end-1);
Grid.dof_ymax_vt = Grid.dof_ymax_vx(2:end-1);

% Pressures on bnd's
Grid.dof_xmin_p = Grid.p.dof_xmin;
Grid.dof_xmax_p = Grid.p.dof_xmax;
Grid.dof_ymin_p = Grid.p.dof_ymin;
Grid.dof_ymax_p = Grid.p.dof_ymax;
...
Grid.dof_pene = sort(Grid.dof_pene);
...
Grid.dof_slip = sort(Grid.dof_slip);
